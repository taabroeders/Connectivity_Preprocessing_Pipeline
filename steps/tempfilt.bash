...
exec 3> func/${FULLID_folder}/temporal_filtering/code_trace.txt
BASH_XTRACEFD=3
...
if [ ${advanced_tempfilt} -eq 1 ];then
        #Select MC file
        if [ -f func/${FULLID_folder}/DisCo/output/rBOLD.par ];then
                nvols=$(fslval func/${FULLID_folder}/ICA_AROMA/denoised_func_data_nonaggr.nii.gz dim4)
                tail -n -${nvols} func/${FULLID_folder}/DisCo/output/rBOLD.par \
                > func/${FULLID_folder}/DisCo/output/rBOLD_nodummy.par
                MCfile=func/${FULLID_folder}/DisCo/output/rBOLD_nodummy.par
        else
