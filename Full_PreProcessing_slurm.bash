...
Optional arguments:
  ...
  --freesurfer_long_reg <sub#_base_to_sub#_ses#.lta> if you are using a longitudinal freesurfer run, the registration from sub#_base to sub#_ses# needs to be provided. 
  ...
  --brain-mask <brain-mask>                          use brain mask (in T1 space) default=freesurfer brain extraction
  ...
  --func_sdc_oppPE                                   perform distortion correction on the functional data based on oposite phase encoding scans
  ...
  --dwi_skullreg				     Use DWI & T1 without skull stripping for registration
Flags:
...
sub-<subject#>/                                      <-- This is the <input-folder>
  ses-<session#>[optional]                           <-- If available, use this as the <input-folder>
    ...
    func/[optional]
      sub-<subject#>[_ses-<session#>]_task-rest*_bold.nii.gz
      sub-<subject#>[_ses-<session#>]_task-rest*_bold.json               ['SliceTiming' field used for slice_time correction]
    dwi/[optional]
...
freesurfer_long_reg='NA' #location of longitudinal freesurfer registration file
lesionmask='' #location of lesion mask
lesionfilled='' #location of lesion-filled T1
brainmask='NA' #location of brain mask in T1-space
...
func_sdc_oppPE='' #perform fieldmap-based distortion correction on functional data
...
dwi_skullreg=0 #perform registration between T1 & DWI on images without skull extraction
...
while [ $# -gt 0 ] ; do
  case $1 in
    ...
    --freesurfer-long-reg | --freesurfer-long-reg) freesurfer_long_reg=$(realpath "$2"); shift ;;
    --lesion-mask | --lesion_mask) lesionmask=$(realpath "$2"); shift ;;
    --lesion-filled | --lesion_filled) lesionfilled=$(realpath "$2"); shift ;;
    --brain-mask | --brain_mask) brainmask=$(realpath "$2"); shift ;;
    --func-sdc | --func_sdc) func_sdc=1 ;;
    --func-sdc-oppPE | --func_sdc_oppPE) func_sdc_oppPE=$(realpath "$2"); shift  ;;
    --dwi-sdc | --dwi_sdc) dwi_sdc=1 ;;
    --dwi-sdc-fmap | --dwi_sdc_fmap) dwi_sdc_fmap=1 ;;
    --dwi_skullreg | --dwi-skullreg) dwi_skullreg=1 ;;
    -h|-\?|--help) print_usage ;;
...
##get input file filters
scriptfolder=$(dirname $(realpath $0))
anat_filt=$(sed "11q;d" ${scriptfolder}/input_filters.txt)
func_filt=$(sed "14q;d" ${scriptfolder}/input_filters.txt)
dwi_filt=$(sed "17q;d" ${scriptfolder}/input_filters.txt)

##set filenames in accordance with folder structure
anatomical_raw=${input_folder}/anat/${FULLID_file}*${anat_filt}*_T1w.nii.gz
...
fmri=${input_folder}/func/${FULLID_file}*${func_filt}*_bold.nii.gz
...
if [[ ${a_flag} -eq 1 ]]; then
  ...
  if [ -z ${lesionfilled} ] && [ ! -z ${anatomical_raw} ] && [ $(ls ${anatomical_raw} | bc -l) -gt 1 ]; then
    printf "ERROR: Multiple instances of anatomical input data were found, adapt the input filters (see input_filters.txt).\n\n"; print_help
  fi
fi
...
if [[ ${f_flag} -eq 1 ]]; then
  if [ ! -f ${fmri} ] || [ ! -f ${fmri%%.nii.gz}.json ];then
    ...
    if [ ! -z ${fmri} ] && [ $(ls ${fmri} | bc -l) -gt 1 ]; then
      printf "ERROR: Multiple instances of functional input data were found, adapt the input filters (see input_filters.txt).\n\n"; print_help
    fi
    if [[ ! -z ${func_sdc_oppPE} ]] && [ ! -f ${func_sdc_oppPE} ]; then
      printf "ERROR: fieldmap-based distortion correction requested for fMRI, but the file was not found";print_help
    fi
  fi
fi
...
if [[ ${d_flag} -eq 1 ]];then
  ...
  if [ ! -z ${dwi} ] && [ $(ls ${dwi} | wc -l) -gt 1 ] ; then
    printf "ERROR: Multiple instances of diffusion data were found, adapt the input filters (see input_filters.txt).\n\n"; print_help
  fi
fi
...
if [ -f ${output_folder}/anat/${FULLID_folder}/atlas/${FULLID_file}_BNA2highres_FIRST.nii.gz ]; then
  ...
  sbatch --wait ${scriptfolder}/steps/fs_to_anat.bash ${anatomical} ${anatomical_noneck} ${anatomical_brain} ${brainmask} ${freesurfer_folder} ${FULLID_folder} ${FULLID_file} &&\
  echo "  Performing hybrid 5TT segmentations..." &&\
  sbatch --wait ${scriptfolder}/steps/hsvs_5ttgen.bash ${anatomical_noneck} ${freesurfer_folder} ${freesurfer_long_reg} ${FULLID_folder} ${FULLID_file} ${scriptfolder} ${lesionmask} &&\
...
if [ -f ${output_folder}/func/${FULLID_folder}/${FULLID_file}_BNA_timeseries.txt ]; then
  ...
  echo "  Performing distortion correction..." &&\
  sbatch --wait ${scriptfolder}/steps/func_sdc.bash ${anatomical_brain} ${fmri} ${scriptfolder} ${FULLID_file} ${FULLID_folder} ${remove_vols} ${func_sdc_oppPE} || print_error ${output_folder} ${FULLID_folder}
  fi
...
if [ -f ${output_folder}/dwi/${FULLID_folder}/tractography/atlas/${FULLID_file}_BNA_Atlas_FA.csv ]; then
  ...
  sbatch --wait ${scriptfolder}/steps/anat_to_dwi.bash ${FULLID_file} ${FULLID_folder} ${scriptfolder} ${dwi_skullreg} &&\
